...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo ""
      echo "Upgrades DECLARED Homebrew formulae only — never casks, so the job"
      echo "never needs root. Outdated casks and undeclared packages are"
      echo "reported; outdated casks are prefetched for a later rebuild.sh run."
